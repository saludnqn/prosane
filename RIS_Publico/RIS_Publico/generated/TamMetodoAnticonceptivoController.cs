using System; 
using System.Text; 
using System.Data;
using System.Data.SqlClient;
using System.Data.Common;
using System.Collections;
using System.Collections.Generic;
using System.ComponentModel;
using System.Configuration; 
using System.Xml; 
using System.Xml.Serialization;
using SubSonic; 
using SubSonic.Utilities;
namespace DalRis
{
    /// <summary>
    /// Controller class for TAM_MetodoAnticonceptivo
    /// </summary>
    [System.ComponentModel.DataObject]
    public partial class TamMetodoAnticonceptivoController
    {
        // Preload our schema..
        TamMetodoAnticonceptivo thisSchemaLoad = new TamMetodoAnticonceptivo();
        private string userName = String.Empty;
        protected string UserName
        {
            get
            {
				if (userName.Length == 0) 
				{
    				if (System.Web.HttpContext.Current != null)
    				{
						userName=System.Web.HttpContext.Current.User.Identity.Name;
					}
					else
					{
						userName=System.Threading.Thread.CurrentPrincipal.Identity.Name;
					}
				}
				return userName;
            }
        }
        [DataObjectMethod(DataObjectMethodType.Select, true)]
        public TamMetodoAnticonceptivoCollection FetchAll()
        {
            TamMetodoAnticonceptivoCollection coll = new TamMetodoAnticonceptivoCollection();
            Query qry = new Query(TamMetodoAnticonceptivo.Schema);
            coll.LoadAndCloseReader(qry.ExecuteReader());
            return coll;
        }
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public TamMetodoAnticonceptivoCollection FetchByID(object IdMetodoAnticonceptivo)
        {
            TamMetodoAnticonceptivoCollection coll = new TamMetodoAnticonceptivoCollection().Where("idMetodoAnticonceptivo", IdMetodoAnticonceptivo).Load();
            return coll;
        }
		
		[DataObjectMethod(DataObjectMethodType.Select, false)]
        public TamMetodoAnticonceptivoCollection FetchByQuery(Query qry)
        {
            TamMetodoAnticonceptivoCollection coll = new TamMetodoAnticonceptivoCollection();
            coll.LoadAndCloseReader(qry.ExecuteReader()); 
            return coll;
        }
        [DataObjectMethod(DataObjectMethodType.Delete, true)]
        public bool Delete(object IdMetodoAnticonceptivo)
        {
            return (TamMetodoAnticonceptivo.Delete(IdMetodoAnticonceptivo) == 1);
        }
        [DataObjectMethod(DataObjectMethodType.Delete, false)]
        public bool Destroy(object IdMetodoAnticonceptivo)
        {
            return (TamMetodoAnticonceptivo.Destroy(IdMetodoAnticonceptivo) == 1);
        }
        
        
    	
	    /// <summary>
	    /// Inserts a record, can be used with the Object Data Source
	    /// </summary>
        [DataObjectMethod(DataObjectMethodType.Insert, true)]
	    public void Insert(string Nombre)
	    {
		    TamMetodoAnticonceptivo item = new TamMetodoAnticonceptivo();
		    
            item.Nombre = Nombre;
            
	    
		    item.Save(UserName);
	    }
    	
	    /// <summary>
	    /// Updates a record, can be used with the Object Data Source
	    /// </summary>
        [DataObjectMethod(DataObjectMethodType.Update, true)]
	    public void Update(int IdMetodoAnticonceptivo,string Nombre)
	    {
		    TamMetodoAnticonceptivo item = new TamMetodoAnticonceptivo();
	        item.MarkOld();
	        item.IsLoaded = true;
		    
			item.IdMetodoAnticonceptivo = IdMetodoAnticonceptivo;
				
			item.Nombre = Nombre;
				
	        item.Save(UserName);
	    }
    }
}

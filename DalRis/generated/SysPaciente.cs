	...
	public partial class SysPaciente : ActiveRecord<SysPaciente>, IActiveRecord
	{
		...
		private static void GetTableSchema() 
		{
			if(!IsSchemaInitialized)
			{
				...
				TableSchema.TableColumn colvarFallecido = new TableSchema.TableColumn(schema);
				colvarFallecido.ColumnName = "fallecido";
				colvarFallecido.DataType = DbType.Boolean;
				colvarFallecido.MaxLength = 0;
				colvarFallecido.AutoIncrement = false;
				colvarFallecido.IsNullable = false;
				colvarFallecido.IsPrimaryKey = false;
				colvarFallecido.IsForeignKey = false;
				colvarFallecido.IsReadOnly = false;
				
						colvarFallecido.DefaultSetting = @"((0))";
				colvarFallecido.ForeignKeyTableName = "";
				schema.Columns.Add(colvarFallecido);
		...
		[XmlAttribute("Fallecido")]
		[Bindable(true)]
		public bool Fallecido 
		{
			get { return GetColumnValue<bool>(Columns.Fallecido); }
			set { SetColumnValue(Columns.Fallecido, value); }
		}
		...
		private DalRis.SysDefuncionCollection colSysDefuncionRecords;
		public DalRis.SysDefuncionCollection SysDefuncionRecords
		{
			get
			{
				if(colSysDefuncionRecords == null)
				{
					colSysDefuncionRecords = new DalRis.SysDefuncionCollection().Where(SysDefuncion.Columns.IdPaciente, IdPaciente).Load();
					colSysDefuncionRecords.ListChanged += new ListChangedEventHandler(colSysDefuncionRecords_ListChanged);
				}
				return colSysDefuncionRecords;			
			}
			set 
			{ 
					colSysDefuncionRecords = value; 
					colSysDefuncionRecords.ListChanged += new ListChangedEventHandler(colSysDefuncionRecords_ListChanged);
			}
		}
		
		void colSysDefuncionRecords_ListChanged(object sender, ListChangedEventArgs e)
		{
		    if (e.ListChangedType == ListChangedType.ItemAdded)
		    {
		        // Set foreign key value
		        colSysDefuncionRecords[e.NewIndex].IdPaciente = IdPaciente;
		    }
		}
        ...
        public static TableSchema.TableColumn FallecidoColumn
        {
        ...
        public void SetPKValues()
        {
                ...
                if (colSysDefuncionRecords != null)
                {
                    foreach (DalRis.SysDefuncion item in colSysDefuncionRecords)
                    {
                        if (item.IdPaciente != IdPaciente)
                        {
                            item.IdPaciente = IdPaciente;
        ...
        public void DeepSave()
        {
            Save();
                ...
                if (colSysDefuncionRecords != null)
                {
                    colSysDefuncionRecords.SaveAll();
               }
		}
        #endregion
	}
